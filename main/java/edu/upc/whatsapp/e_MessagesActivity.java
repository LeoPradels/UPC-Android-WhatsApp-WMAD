...
public class e_MessagesActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    System.out.println("On Create Message.");
    setContentView(R.layout.e_messages);
    globalState = (_GlobalState) getApplication();
    System.out.println("globalState user to talk to:"+globalState.user_to_talk_to+" user to talk id :"+globalState.user_to_talk_to.getId());
    System.out.println("globalState my user:"+globalState.my_user);
  ...
  public void sendText(final View view) {
    ...
    mes.setContent(input_text.getText().toString());
    mes.setDate(new  Date());
    mes.setUserSender(globalState.my_user);
    mes.setUserReceiver((globalState.user_to_talk_to));
    new SendMessage_Task().execute(mes);
    input_text.setText("");
    ...
    input_text.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
      ...
      public void onGlobalLayout() {
        ...
        if (heightDifference > 50 && !enlarged) {
          ...
          enlarged = true;
          shrunk = false;
          conversation.setStackFromBottom(true);
          conversation.post(new Runnable() {
            @Override
            public void run() {
              conversation.smoothScrollToPosition(conversation.getCount() - 1);
